...
<body class="bg-texture text-charcoal font-sans antialiased overflow-x-hidden no-scroll" id="mainBody">
    @php
        // Helper Pintar untuk URL Gambar/Musik
        function getFileUrl($path, $default = 'https://via.placeholder.com/300') {
            // 1. Jika path kosong, pakai default
            if (empty($path)) return $default;
            
            // 2. Jika path adalah URL lengkap (https://...), pakai langsung
            if (filter_var($path, FILTER_VALIDATE_URL)) return $path;
            
            // 3. Jika path SUDAH mengandung 'storage/' di depannya, jangan ditambah lagi
            // (Ini penyebab utama error Anda sebelumnya)
            if (strpos($path, 'storage/') === 0) {
                return asset($path);
            }
            
            // 4. Jika path dimulai dengan 'assets/' (file bawaan tema), pakai langsung
            if (strpos($path, 'assets/') === 0) {
                return asset($path);
            }
            
            // 5. Jika hanya nama file (contoh: foto.jpg), anggap ada di storage
            return asset('storage/' . $path);
        }

        // Logika Musik (Fallback)
        $userMusic = $invitation->content['media']['music'] ?? null;
        if ($userMusic) {
            // Gunakan helper yang sama agar aman
    ...
    {{-- WISHES SECTION (FIXED) --}}
    <section class="relative bg-texture pt-32 pb-32 px-6">
        
        {{-- PERBAIKAN: Tambahkan 'pointer-events-none' agar gelombang tidak memblokir klik --}}
        ...
        {{-- PERBAIKAN: Pastikan z-index tinggi (z-20) agar form berada di ATAS elemen lain --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-10">
                ...
                <div>
                    <form action="{{ route('kirim.ucapan') }}" method="POST" class="space-y-4">
                        ...
                        {{-- Input Nama --}}
                        <div class="relative z-30"> {{-- Tambahan z-30 untuk memastikan input bisa diklik --}}
                    ...
                    <div class="rustic-scroll h-[400px] overflow-y-auto pr-2">
                        @if($invitation->comments->count() > 0)
                            @foreach($invitation->comments->sortByDesc('created_at') as $comment)
                                <div class="rustic-comment-card group bg-white/50 backdrop-blur-sm"> {{-- Tambah bg agar lebih terbaca --}}
        ...
        {{-- WAVE BAWAH (Juga dikasih pointer-events-none) --}}
